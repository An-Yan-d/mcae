...
def char(c,t,p,s=3):
    ...
    print(c)
    default = ('end_rod', 0, 0, 0, 0, 1)
    f=font.Font(r'../fonts/Gabriola.ttf')
    ps=f.point_generation(c,step=100,size=s,xrange=(-200,2200),yrange=(-600,2600))
    motions=motion.CmdBuilder()
    ps=tool.move(ps,*p)
...
def String1(t):
    ...
    for i in range(n):
        if s[i]!=' ':
            result+=char(s[i],t.st+int(i*t.dt/n/2),[sp1+i*d,85,132])
    return result
...
def picture(t,hsize,center,step=0.5):
    default = ('end_rod', 0, 0, 0, 0, 1)
    img=cv2.imread(r'images/]9EJ`13ANYQ~$9}CUZ1W}54.png',0)
    points=[]
    (h,w)=img.shape
    ratio=w/h
    wsize=ratio*hsize
...
CMD +=String1(S[25])+firework2(S[24], [393, 66,130],offset=0.75,s=0.25)
CMD +=picture(S[24],30,[370.5,90,130])
...
# T.newgroupstart(110)
# T.newplan(0,100)
# CMD +=String1(T[1])+firework2(T[0], [393, 66,130],offset=0.75,s=0.25)
# CMD +=picture(T[0],30,[370.5,90,130])
